...
|---|--------|------|-----------|---------------|----------|-------|
| 1 | SEC EDGAR RSS | rss | T1 | 30 min | 90 | Official regulatory filings |
...
- T1 sources (SEC, Fed, Treasury) polled every 30 min — official releases are
  infrequent but high-importance; 30 min is sufficient.
- T2 sources (Reuters, CNBC) polled every 15 min — wire services publish
  frequently; 15 min provides timely coverage without excessive API load.
- T3 sources (Yahoo Finance, MarketWatch) polled every 15 min — same cadence
  as wire for consistent ingestion cycles.
...
| 7 | CoinGecko API | api | T1 | 15 min | 90 | Market data (inactive until parser ready) |

> Note: CoinGecko API is listed in config but `is_active: 0` in D1 seed until
> parser mapping is implemented. Once enabled, it becomes the highest-priority
> source.

### Inactive / Pending Sources (7)

| # | Source | Type | Trust Tier | Reason Inactive |
|---|--------|------|-----------|-----------------|
| 1 | CoinGecko API | api | T1 | Parser mapping not yet implemented |
| 2 | NewsAPI Crypto | newsapi | T3 | Requires NEWS_API_KEY subscription |
| 3 | Whale Alert (X) | x_account | T4 | Pending X API credentials |
| 4 | CZ Binance (X) | x_account | T4 | Pending X API credentials |
| 5 | Vitalik Buterin (X) | x_account | T4 | Pending X API credentials |
| 6 | X Search: BTC Breakout | x_query | T4 | Pending X API credentials |
| 7 | X Search: ETH DeFi | x_query | T4 | Pending X API credentials |
...
- T2 source (Reuters) polled every 15 min — provides editorial credibility.
- T3 specialist sources polled every 10 min — crypto moves fast; shorter
  interval catches breaking news from specialist outlets sooner.
- T4 X sources (when enabled) poll every 5–10 min — social signals are
  time-sensitive but capped at severity 60.
...
- [x] Crypto active source list defined (6 RSS + 1 API pending parser)
- [x] Trust tier assignments reviewed and documented
- [x] Poll intervals set per source category
- [x] Priority weights assigned for ingestion ordering
- [x] Noisy/low-value sources explicitly excluded with reasoning
- [x] X sources configured but inactive pending credentials
- [x] NewsAPI source configured but inactive pending API key
