...
INSERT OR IGNORE INTO sources
  (source_slug, source_name, topic_slug, source_type, trust_tier, trust_score, priority_weight, url, is_active, poll_interval_minutes, ingestion_method, metadata_json)
VALUES
  ('sec-edgar-rss', 'SEC EDGAR RSS', 'finance', 'rss', 'T1', 90, 90, 'https://www.sec.gov/cgi-bin/browse-edgar?action=getcurrent&type=8-K&dateb=&owner=include&count=20&search_text=&output=atom', 0, 30, 'poll', '{"notes":"Atom feed — module 01 RSS parser must support <entry> elements for items to be ingested. Set is_active=1 after parser is extended."}'),
