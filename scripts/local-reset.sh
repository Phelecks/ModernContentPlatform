...
echo ""
echo "  NOTE: build the Vue app at least once before starting the server:"
echo "    cd app && npm run build && cd .."
echo ""
echo "  Run the full stack (two terminals, hot reload):"
echo "    Terminal 1: wrangler pages dev app/dist --d1 DB=YOUR_PRODUCTION_D1_DATABASE_ID"
echo "    Terminal 2: cd app && npm run dev"
echo ""
echo "  Or run the built stack (single server, no hot reload):"
echo "    wrangler pages dev app/dist --d1 DB=YOUR_PRODUCTION_D1_DATABASE_ID"
echo "========================================"
echo ""
